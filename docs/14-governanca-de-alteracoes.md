...
> **Estado**: 🔮 Planejado (sessões S26–S34) · **DECs relacionadas**: DEC-GOV-001 a DEC-GOV-009
> em [`13-decisoes.md`](13-decisoes.md) · Normativo para as sessões do épico; os contratos de
> schema fecham na S26 e passam a viver em [`03-modelo-do-documento.md`](03-modelo-do-documento.md).
...
## 3. Conceitos e modelo (rascunho normativo — fecha na S26)
...
`schemaVersion: 4`, migração 3→4 **puramente aditiva**: `components: []`, `changeRequests: []`,
`releases: []`, e novos kinds de catálogo (`MOTIVATOR`, `IMPACT_CATEGORY`). Nenhum campo existente
muda de forma. Nota de migração em `03-modelo-do-documento.md` §10 (S26).
...
1. **Numeração dos DBs** — seguir a sequência atual da área (DB-520 em diante) ou reiniciar?
   Proposta: campo livre com sugestão sequencial a partir do maior número existente. (S29)
