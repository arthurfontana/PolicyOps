# Sessão 31 — Release de política e timeline do Diário de Bordo

**Modelo:** `Sonnet` · **Depende de:** S30 · **Épico/Marco:** Governança (M9)

> **Por que Sonnet:** a publicação atômica (a parte perigosa) veio pronta da S30; release é CRUD +
> composição sobre ela, e a timeline é leitura. Contratos fechados em `docs/14` §3.4 e RN-GOV-05.

## Prompt

> Você está implementando a Sessão 31 do PolicyOps — releases de política e a visão cronológica
> do Diário de Bordo.
>
...
>
> ### Estado atual
> S30 entregou `changeRequest/publish` atômico e o vínculo item↔rascunho. `Release` existe no
> schema (S26) sem tela. A timeline de matriz (S15) é o padrão visual de referência.
>
> ### Objetivo
...
>
> ### Fora do escopo
> Comparação release × release (S33), pacote (S32), notificações além do painel de pendências.
>
> ### Atualização da documentação (obrigatório)
> No mesmo PR: `docs/14-governanca-de-alteracoes.md` (US-GOV-05 ✅), `docs/07-ux-e-editor.md`
> (release e timeline), `docs/13-decisoes.md`, linha da S31 em `docs/09`.
>
> ### Regras transversais
