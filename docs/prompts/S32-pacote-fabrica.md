# Sessão 32 — Pacote para a Fábrica
...
> Você está implementando a Sessão 32 do PolicyOps — a geração automática do Pacote de
...
>
> ### Estado atual
> DBs publicáveis com itens, rascunhos, critérios, testes e vigência (S30). `RichDoc` renderiza no
> editor (S28). Export existente cobre CSV/JSON/PNG/impressão de matriz. `Project` ainda não tem
> `factoryTemplate` na prática (schema da S26 prevê).
>
> ### Objetivo
...
> `docs/10-guia-do-usuario.md` (como gerar o pacote), `docs/13-decisoes.md`, linha da S32 em
> `docs/09`.
>
> ### Regras transversais
