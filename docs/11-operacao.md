...
- **`_evidencias\`** guarda os arquivos anexados às políticas, em estrutura legível (`projeto\matriz\versão\`, nomes com prefixo de data: `2026-08-14_DB 513 - Ajuste.docx`). **Nunca renomeie nem mova arquivos aí dentro** — o documento aponta para esses caminhos e confere o conteúdo por hash: um arquivo renomeado vira "não encontrado" na aplicação (com o caminho esperado na mensagem), e um arquivo trocado por outro vira um aviso de conteúdo divergente. Os dois casos se resolvem devolvendo o arquivo original ao lugar. Excluir anexo pela aplicação apenas move o arquivo para `_evidencias\_lixeira\`, preservando a mesma árvore de pastas; esvaziar a lixeira é decisão manual de quem opera a pasta. A pasta só é criada quando alguém anexa a primeira evidência.
