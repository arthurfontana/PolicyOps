...
44 sessões incrementais — as 17 do MVP, 18–20 pós-MVP (biblioteca de variáveis), 21–25 do épico Carga, 26–31 do épico **Plataforma** (`14-plataforma-local.md`), 32–40 do épico **Governança de Alterações** (`14-governanca-de-alteracoes.md`, ✅ concluído — 32a/32b/33a/33b/34/35/36/37/38/39/40 entregues) e 41–44 do épico **Experiência** (reforma de layout depois do uso real: uma árvore só, o componente no centro, ações na barra de ferramentas e a consulta histórica como tela — `07-ux-e-editor.md` §2/§2.1/§10/§17/§21, DEC-UX-001 a DEC-UX-005; a 32 foi dividida em **32a**/**32b** e a 33, em **33a**/**33b** — DEC-GOV-010 e DEC-GOV-012). Cada uma termina com algo que roda, testado, commitado, e com `dist/PolicyOps.html` atualizado. A **45** é um adendo fora do lote original, aberta por uso real de carga (limite em faixa por célula, `docs/prompts/S45-limite-em-faixa.md`).
...
| 35 ✅ | Solicitação de Alteração e workflow | **Sonnet** | ✅ **Entregue** — `ChangeRequestsScreen`/`ChangeRequestDetail` (`src/components/change-requests/`): criar DB (código sugerido sequencialmente, I26), motivadores e itens (1..N, "hoje" da versão vigente), impactos/critérios/testes, motivação/especificação (`RichDocEditor`, `RichDocTarget` estendido sem migração), grafo de 12 estados na tela com decisões de aprovação/devolução/rejeição (`DecisionDialog`, comentário obrigatório na devolução), fila de aprovação (a própria lista filtrada) e painel de pendências no documento (`ChangeRequestPendingPanel`) — 100% testado (`tests/unit/components/change-requests/`, `tests/e2e/diario-de-bordo.spec.ts` cobrindo CT-GOV-01 parcial e CT-GOV-02 com o DB-515 real) | 32b, 33, 34 |
| 36 ✅ | Vínculo DB ↔ rascunhos e publicação | **Opus** | ✅ **Entregue** — `changeRequest/linkDraft`/`unlinkDraft`/`createComponentItem` (`src/core/document/cr-drafts.ts`: cria ou adota o rascunho, um rascunho por DB, desvincular não descarta sem confirmação, item `CREATE` cria componente+item+rascunho numa transação) e `changeRequest/publish` (`cr-publish.ts`: valida tudo antes de tocar, publica componentes **e** matrizes com a vigência do DB numa operação atômica no padrão de `import/apply`, `E-GOV-04` com a lista e `E-GOV-02` exigindo rebase reconfirmado item a item); congelamento do rascunho vinculado de ponta a ponta (I25, `cr-freeze.ts`, guardas em `versioning/` e `richdoc/`); diff de payload campo a campo (`diff/component-payload.ts`) + spec por bloco + `CompareView` no item de matriz; `PublishChangeRequestDialog` e o vínculo na linha do item (docs/07 §19.5) — CT-GOV-01 completo, CT-GOV-02, CT-GOV-03 (individual), CT-GOV-04 e CT-GOV-06 verdes | 35 |
| 37 ✅ | Release e timeline do Diário de Bordo | **Sonnet** | ✅ **Entregue** — `release/publish` (`src/core/document/release-publish.ts`): roda `changeRequest/publish` (S36) por DB pronto da release, numa transação só — valida todos antes de tocar, `E-GOV-04` com a lista agregada, cada DB mantém a sua vigência (CT-GOV-03 exato); vínculo `changeRequest/setRelease` passou a exigir DB ≥ APPROVED para entrar (sair continua livre até fechar); `deriveReleaseJointStatus` (EMPTY/IN_PROGRESS/READY/PUBLISHED/CANCELLED); telas `ReleasesScreen`/`ReleaseDetail`/`PublishReleaseDialog` (docs/07 §19.6) e `ChangeRequestTimelineScreen` (US-GOV-08 parcial, docs/07 §19.7) — 100% testado (`release-publish.test.ts`, `ReleaseDetail.test.tsx`, `ChangeRequestTimelineScreen.test.tsx`, `tests/e2e/release-e-timeline.spec.ts` com o cenário "release 2026.09.01 — DB-515, DB-519") | 36 |
| 38 ✅ | Pacote para a Fábrica | **Sonnet** | ✅ **Entregue** — gerador puro `buildFactoryPackage` (`src/core/export/factory-package.ts`): DB + `factoryTemplate` do projeto + documento → estrutura na ordem do §8 (identificação, registro de versão, contatos, boilerplate, contexto, escopo "hoje × proposto" por item, impactos, critérios, testes, vigência, anexos), disponível a partir de `APPROVED` e sempre regenerado (RN-GOV-08); dois renderizadores puros (`factory-package-html.ts`, `factory-package-markdown.ts` — imagem embutida em base64 no HTML, referenciada por nome de anexo + aviso no Markdown); `Project.factoryTemplate` (boilerplate `RichDoc` + contatos) editável em "Editar projeto" com um alvo novo de `RichDocTarget` (`PROJECT_FACTORY_BOILERPLATE`); botão "Gerar pacote" no DB e "Gerar pacotes" (lote) na release — 100% testado (`tests/unit/core/export/factory-package.test.ts` com snapshot do DB-515, `tests/unit/core/richdoc/commands.test.ts`, `tests/unit/core/document/commands.test.ts`) | 36 |
| 39 ✅ | Fotografia histórica da política | **Opus** | ✅ **Entregue** — `getPolicyAt` (`src/core/timeline/policy-at.ts`): a política inteira numa data, compondo componentes e matrizes (DEC-GOV-002), com as bordas explícitas (vigência semiaberta, publicação agendada que ainda não vige, arquivado que permanece nas fotografias anteriores, seção pura como estrutura por I29, matriz sem espelho como nó de raiz — DEC-GOV-039), contadores do período e janela de comparação por release (DEC-GOV-040); `diffPolicySnapshots` (`src/core/diff/policy-diff.ts`) casa as duas fotografias e classifica ADDED/REMOVED/CHANGED reaproveitando os três motores existentes (payload campo a campo, `RichDoc` por bloco, matriz resumida), agregado por seção da árvore; telas: modo fotografia da árvore ("ver como em…", edição bloqueada), salto a partir da timeline do componente e `PolicyCompareScreen` com data × data e release × release (docs/07 §20) — 100% no núcleo novo, CT-GOV-01 (parte final) verde e fotografia + comparação de 402 componentes e 102 matrizes em < 500 ms | 36 |
...
| 41 ✅ | Shell: navegação e barra de ferramentas | **Opus** | ✅ **Entregue** — a árvore da política passou a ser renderizada **dentro** da `Sidebar` (todos os níveis, `sidebarTreeAnchors` e o painel de 360px removidos), com largura arrastável de 248 a 480px (alça de 4px, duplo clique restaura 288px, `←`/`→` movem 16px, persistida em `policyops.sidebarWidth` com validação na leitura — DEC-UX-005) e o `code` do nó só a partir de 340px; slot de barra de ferramentas no shell (`ToolbarPortal`/`ToolbarSlot`, `src/components/shell/Toolbar.tsx`) preenchido por portal pela `PolicyTree` enquanto a tela da política está ativa (DEC-UX-006), com criação (Nova seção · Nova regra · Pendurar matriz · Carregar Markdown), Publicar pendentes (n), "ver como em…" (transitório até a S43), busca, `Filtrar (n)` em popover e chip de alvo `em: … › nó`; colapso em `⋯ Mais` abaixo de ~1100px **ou** quando a linha não couber; menu `⋯` do nó ganhou Nova regra e Arquivar, mais `Recolher tudo`/`Expandir tudo` e `Alt+clique` no chevron; a nota de ordem de leitura virou `title` do cabeçalho do projeto e entrada do diálogo de atalhos — CT-UX-01/03/05 verdes (`tests/e2e/navegacao-e-barra-de-ferramentas.spec.ts`) | 33b, 40 |
| 42 ✅ | Página do componente no centro | **Sonnet** | ✅ **Entregue** — `ComponentPage` (`src/components/tree/ComponentPage.tsx`) substitui `ComponentContentPanel` + `ComponentInspector`: regra, seção e lista se editam em coluna de até 960px no centro — cabeçalho com nome em edição inline e navegação entre irmãos (`Alt+↑`/`Alt+↓`), barra de ações grudenta (estado + `Salvando…`/`Salvo` + ciclo de vida + `⋯`), identidade em grade de 2 colunas, payload, especificação e vigência na mesma rolagem; `ComponentPayloadFields` ganhou `layout: 'wide' \| 'narrow'` (campo longo com `AutoGrowTextarea` ≥6 linhas, curtos em par) e os campos de lista (`inputs`/`reasonCodes`/`dependencies`) viram chips; `Inspector.tsx` não roteia mais componente e o shell some com o `aside` direito nas telas sem seleção (`useHasInspector`) — CT-UX-02 verde (`tests/e2e/pagina-do-componente.spec.ts`), cobertura migrada para `tests/unit/components/tree/ComponentPage.test.tsx` (docs/07 §6, §17.5; DEC-UX-002) | 41 |
| 43 ✅ | Vigência: a política inteira na data | **Opus** | ✅ **Entregue** — a tela de Vigência ganhou três abas sobre a mesma data (**Estrutura** padrão, **Matrizes** e **Portfólio**): a Estrutura desenha a árvore do projeto na data a partir de `getPolicyAt` (`buildStructureRows`, `src/components/timeline/policy-structure-rows.ts`) com `v{n}`, "sem política vigente em dd/mm/aaaa" e `estrutura` (I29), some com o que não existia na data e reaproveita o nó da árvore editável (`TreeNodeRow`, extraído da `PolicyTree`); selecionar um nó abre ao lado o conteúdo **daquela** versão em somente leitura — payload campo a campo e especificação rica, ou o grid da versão vigente na matriz —, com banner de versão histórica, marca d'água e **"Abrir no editor (hoje)"**; barra de ferramentas própria no slot do shell (data com os atalhos, projeto, busca e `Filtrar (n)` da aba Estrutura, "Comparar com outra data" já preenchendo uma ponta). O **modo fotografia da árvore saiu** (DEC-UX-004): `componentTree.snapshotDate` e todo o `readOnly` derivado dele sumiram da árvore e da página do componente, e entrou `ui-store.policyAtDate`/`openPolicyAt`, que os dois saltos (faixa de vigência do componente, "Ver a política em…" da comparação) usam — CT-UX-04 verde (`tests/e2e/fotografia-da-politica.spec.ts`), cobertura das quatro leituras migrada para `tests/unit/components/timeline/`, e abrir a data com 300 componentes e 102 matrizes em < 500 ms (fixture da S39, agora em `tests/unit/core/document/large-policy-fixture.ts`) | 42 |
| 44 ✅ | Fluxo do dia a dia — teclado/volume | **Sonnet** | ✅ **Entregue** — a árvore ganhou navegação completa por teclado (roving tabindex, `role="treeitem"`/`aria-level`/`aria-expanded`, `↑`/`↓`/`→`/`←`/`Home`/`End`, typeahead, `Delete` arquiva com confirmação — `flattenComponentTree`/`nextVisibleNodeId`/`previousVisibleNodeId`/`findTypeaheadMatch` em `src/core/queries.ts`); contador amplo de pendentes na barra (`n pendentes`, rascunho aberto **ou** `PENDING_REVIEW` — `listPendingComponents`) abre "Publicar pendentes" ao clicar, e `Ctrl+Shift+N` (`nextPendingComponentId`) salta para o próximo pendente da política, dando a volta e desarmado com zero pendentes; densidade: "Itens filhos" saiu da página do componente (já está na árvore ao lado) e a árvore ganhou estado vazio de filtro sem resultado; diálogo de atalhos reorganizado por região de tela (barra, árvore, página do componente) — CT-UX-03 completo, teste de volume (`tests/e2e/cadastro-em-volume.spec.ts`, `tests/unit/components/tree/PolicyTree.test.tsx`, `tests/unit/core/queries.test.ts`) e acessibilidade da árvore por teclado (`tests/e2e/acessibilidade.spec.ts`) | 43 |
| 45 | Limite em faixa (mín/máx) na célula | **Sonnet** | `Cell.limitMin`/`limitMax`, seletor "Valor único" vs "Faixa" no inspector, papéis `limitMin`/`limitMax` no import — sem catálogo forçado por valor único observado | 12, 22 |
...
> **Sessões 32–40 são o épico Governança de Alterações** ([`14-governanca-de-alteracoes.md`](14-governanca-de-alteracoes.md), decisões `DEC-GOV-*` em [`13-decisoes.md`](13-decisoes.md), ✅ concluído — lotes 1–5 entregues (32b–39 ✅), ver tabela abaixo): a política inteira como árvore de componentes versionados, o Diário de Bordo como Solicitação de Alteração estruturada com workflow, releases e Pacote para a Fábrica gerado. Nasceu com numeração própria (26–34) numa sessão de especificação separada da do épico Plataforma e foi renumerado para 32–40 ao consolidar as duas — inclusive o schema, que sai de `schemaVersion: 4` para `schemaVersion: 5` porque a 4 já é do épico Plataforma (`meta.acl`, S29; ver DEC-GOV-009). As perguntas abertas do §12 de `14-governanca-de-alteracoes.md` devem ser fechadas antes das sessões que as citam.
>
...
| **5 — Governança** ✅ | 34 ✅ → 32b ✅ → 35 ✅ → 36 ✅ → 37 ✅/38 ✅ → 39 ✅ | Editor rico, Diário de Bordo, publicação com vigência, release, pacote e fotografia histórica (32b–39 entregues — ver linhas próprias acima) |
...
| **M13 — Layout do uso diário** | 44 | A política se navega numa árvore só, a regra se escreve na largura da tela, a consulta histórica tem tela própria e o cadastro em volume roda no teclado |
