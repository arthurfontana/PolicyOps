...
| I9 | `RANGE` sem `groupingDimensions`: no máximo um `isCatchAll`, e ele é o último por `position` — isso é **ERROR**, bloqueante. A contiguidade de valores em si (faixas sem buraco nem sobreposição, usando `rangeMin`/`rangeMax`) é **WARNING**: não bloqueia `saveDomains` nem `publish`, só alimenta avisos na interface (docs/07-ux-e-editor.md §11) — um score cujas faixas ainda não foram fechadas, ou importado aos poucos, não fica impedido de salvar por isso. `RANGE` **com** `groupingDimensions`: a identidade do catch-all vale para a variável inteira (independe de `path`); a mesma checagem de contiguidade/sobreposição (WARNING) vale **independentemente para cada `path` distinto** presente em `domains[].groupingRanges` — não para toda combinação possível de opções, só para as que o usuário efetivamente definiu (ver I19). Em ambos os casos, a contiguidade lê `boundaryMode` da versão: `INCLUSIVE_INTEGER` (default) exige valores inteiros e `atual.máx + 1 == próxima.mín`; `HALF_OPEN` exige `atual.máx == próxima.mín`. A ordem por `position` pode ser crescente ou decrescente — a direção é detectada pelo primeiro par de mínimos distintos e aplicada à sequência inteira (ou a cada `path`, com `groupingDimensions`) |
