...
| Consultas de leitura (vigência, timeline, auditoria; a política numa data e a comparação entre duas — núcleo S39 ✅, tela de Vigência com as abas Estrutura/Matrizes/Portfólio e sem modo fotografia na árvore — S43 ✅ em `docs/07` §10/§20) | `docs/05-regras-de-negocio.md` §6 | `src/core/queries.ts`, `src/core/timeline/policy-at.ts`, `src/core/diff/policy-diff.ts`, `src/components/timeline/`, `src/components/tree/TreeNodeRow.tsx` |
...
| Árvore da política e cadastro/versionamento de regras (S33a/S33b ✅; árvore na barra lateral + barra de ferramentas do shell — S41 ✅ em §2/§2.1/§17.1; página do componente no centro — S42 ✅ em §17.5; teclado/roving tabindex, contador amplo de pendentes e `Ctrl+Shift+N` — S44 ✅ em §17.1) | `docs/07-ux-e-editor.md` §17 | `src/components/tree/{ComponentPage,PolicyTree}.tsx`, `src/components/shell/{Toolbar,Sidebar,Shell,ShortcutsDialog}.tsx`, `src/components/inspector/ComponentPayloadFields.tsx`, `src/core/versioning/component-lifecycle.ts` |
...
| Limite em faixa na célula (mín/máx, alternativa ao valor único de catálogo — S45 ✅) | `docs/03-modelo-do-documento.md` §6.2 | `src/core/document/`, `src/components/inspector/CellInspector.tsx`, `src/core/import/` |
